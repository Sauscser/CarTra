...
import { CountySubCountySetupContent } from './CountySubCountySetupScreen';
...
export default function CountyHomeScreen() {
  return <CountyHomeContent />;
}

function CountyHomeContent() {
  const client = useMemo(() => generateClient(), []);
  const [county, setCounty] = useState<CountyEntity | null>(null);
  const [subCounties, setSubCounties] = useState<SubCountyEntity[]>([]);
  const [countySchools, setCountySchools] = useState<Array<{ id: string; code: string; name: string; subCountyCode?: string | null }>>([]);
  const [countyLearners, setCountyLearners] = useState<Array<{ id: string; fullName: string; gradeLevel?: string | null; classCode?: string | null; assessmentNumber?: string | null; subCountyCode?: string | null; schoolCode?: string | null }>>([]);
  ...
  useEffect(() => {
    void loadCurrentCounty();
  }, []);

  useEffect(() => {
    if (county?.code) {
      void Promise.all([loadCountyLearners(), loadCountyGrade12Summaries()]);
    }
  }, [county?.code]);

  useEffect(() => {
    if (selectedSubCountyCode) {
      void loadCountySchools(selectedSubCountyCode);
    } else {
      setCountySchools([]);
    }
  ...
  const loadCurrentCounty = async () => {
    try {
      ...
      const matchedCounty = countyPayload.data?.listOrgHierarchies?.items?.[0];

      if (!matchedCounty) {
        setCounty(null);
        setSubCounties([]);
        setNotice(`No county is assigned to ${currentEmail}.`);
        return;
      }

      const nextCounty: CountyEntity = {
        id: matchedCounty.id,
        code: matchedCounty.code,
        name: matchedCounty.name,
        nationCode: matchedCounty.nationCode,
        regionCode: matchedCounty.regionCode,
        assignedOfficerEmail: matchedCounty.assignedOfficerEmail,
      };
      ...
      const subCountyResult = await client.graphql({
        ...
        variables: {
          filter: {
            and: [{ entityType: { eq: 'subCounty' } }, { countyCode: { eq: nextCounty.code } }],
          },
          limit: 100,
        },
      });
      ...
      const mappedSubCounties = (subCountyPayload.data?.listOrgHierarchies?.items || [])
      ...
      setSubCounties(mappedSubCounties);
      ...
      setCounty(null);
      setSubCounties([]);
      setNotice('Could not load the county dashboard for this account.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const loadCountySchools = async (subCountyCode: string) => {
    try {
      const result = await client.graphql({
        query: listOrgHierarchies,
      ...
      setCountySchools(items.filter((item: any) => item?.id && item?.code && item?.name).map((item: any) => ({
        id: item.id,
        code: item.code,
        name: item.name,
      ...
      console.warn('loadCountySchools failed', error);
      setCountySchools([]);
    }
  };

  const loadCountyGrade12Summaries = async () => {
    if (!county?.code) {
      setGrade12Summaries([]);
      return;
    }
    ...
    try {
      ...
      const items = ((result as any).data?.listGrade12ResultSummaries?.items || [])
        .filter((item: any) => item?.id && item?.learnerId && item?.schoolCode && String(item.countyCode || '').toLowerCase() === String(county.code || '').toLowerCase())
      ...
      console.warn('loadCountyGrade12Summaries failed', error);
      setGrade12Summaries([]);
    }
  };
  ...
  const modalContent = performanceAction && selectedLearner ? (
    <Modal transparent visible={!!performanceAction} animationType="slide" onRequestClose={resetPerformanceModal}>
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center' }}>
        <View key={`${performanceSessionId}-${selectedLearner?.id ?? 'county-performance'}`} style={{ margin: 16, backgroundColor: '#fff', borderRadius: 12, padding: 12, maxHeight: '90%' }}>
          ...
          {performanceAction === 'viewCluster' ? (
            <ScrollView style={{ marginTop: 8 }}>
              ...
              <View style={{ marginTop: 12 }}>
                <LearnerComments learnerId={selectedLearner.id} currentGrade={selectedLearner.gradeLevel} authorRoleOverride="county" />
              </View>
            </ScrollView>
          ) : null}
  ...
  const loadCountyLearners = async () => {
    ...
    try {
      const subCountyResult = await client.graphql({
        ...
        variables: {
          filter: {
            and: [{ entityType: { eq: 'subCounty' } }, { countyCode: { eq: county.code } }],
          },
          limit: 100,
        },
      });
      ...
      const subCountyPayload = subCountyResult as { data?: { listOrgHierarchies?: { items?: Array<any> } } };
      const subCountyCodes = (subCountyPayload.data?.listOrgHierarchies?.items || [])
      ...
      const learnerBatches = await Promise.all(subCountyCodes.map(async (subCountyCode: string) => {
        const result = await client.graphql({
          query: listLearnerProfiles,
          variables: { filter: { subCountyCode: { eq: subCountyCode } }, limit: 500 },
        } as any);
      ...
      const filteredItems = items.filter((learner: any) => {
        ...
        const includeLearner = !!lastAssessment && lastAssessment.deviation <= -40;

        console.log('[CountyHomeScreen] learner last grade cluster check', JSON.stringify({
      ...
      setCountyLearners(filteredItems.map((learner: any) => ({
        id: learner.id,
      ...
      console.warn('loadCountyLearners failed', error);
      setCountyLearners([]);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <SectionCard title="County Office" subtitle="Your assigned county details and sub-county management.">
          {isLoading ? (
            <View style={styles.loadingState}>
              <ActivityIndicator size="small" color="#1d4ed8" />
              <Text style={styles.loadingText}>Loading your county dashboard...</Text>
            </View>
          ) : null}
          ...
          {!isLoading && county ? (
            <View style={styles.summary}>
              <Text style={styles.heading}>{county.name}</Text>
              <Text style={styles.meta}>Code: {county.code}</Text>
              <Text style={styles.meta}>Region: {county.regionCode || 'N/A'}</Text>
              <Text style={styles.meta}>Nation: {county.nationCode || 'N/A'}</Text>
              <Text style={styles.meta}>Assigned officer: {county.assignedOfficerEmail || 'Not set'}</Text>
            </View>
          ) : null}
        </SectionCard>

        <SectionCard title="Sub-counties in this county" subtitle="All sub-counties linked to your assigned county.">
          {!isLoading && subCounties.length === 0 ? (
            <Text style={styles.empty}>No sub-counties are assigned to this county yet.</Text>
          ) : null}

          {subCounties.map((subCounty) => (
            <TouchableOpacity
              key={subCounty.id}
              style={[styles.row, selectedSubCountyCode === subCounty.code && styles.rowSelected]}
            ...
            >
              ...
              <Text style={styles.rowMeta}>{subCounty.assignedOfficerEmail || 'No sub-county officer assigned'}</Text>
            </TouchableOpacity>
          ))}
          {selectedSubCountyCode ? (
            <View style={styles.selectionBlock}>
              <Text style={styles.selectionTitle}>Schools in {subCounties.find((item) => item.code === selectedSubCountyCode)?.name || selectedSubCountyCode}</Text>
              {!countySchools.length ? <Text style={styles.empty}>No schools found in this sub-county.</Text> : countySchools.map((school) => (
                <TouchableOpacity
                  key={school.id}
        ...
        <SectionCard title="Affected learners" subtitle="Select a sub-county, school, and grade to view affected learners.">
          {!countyLearners.length ? (
            <Text style={styles.empty}>No learners in this county are below target by -40 or more in their last grade yet.</Text>
          ) : !selectedSubCountyCode || !selectedSchoolCode || !selectedLearnerGrade ? (
            <Text style={styles.empty}>Select a sub-county, school, and grade above to view learners.</Text>
          ) : (
            <ScrollView style={{ maxHeight: 720 }} contentContainerStyle={{ paddingBottom: 24 }} showsVerticalScrollIndicator>
              {countyLearners.filter((learner) => {
                const learnerGrade = String(learner.gradeLevel || learner.classCode || '').replace(/\D/g, '');
                return learner.subCountyCode === selectedSubCountyCode && learner.schoolCode === selectedSchoolCode && learnerGrade === selectedLearnerGrade;
              }).map((learner) => (
                <View key={learner.id} style={styles.learnerRow}>
                  ...
                  <Text style={styles.learnerMeta}>{countySchools.find((school) => school.code === learner.schoolCode)?.name || learner.schoolCode || 'School N/A'} • {learner.schoolCode || 'Registration code N/A'}</Text>
                  <Text style={styles.learnerMeta}>Grade {learner.gradeLevel || learner.classCode || 'N/A'} • Class {learner.classCode || 'N/A'} • {learner.subCountyCode || 'N/A'}</Text>
                  <TouchableOpacity
        ...
        <SectionCard title="Grade 12 result summary pool" subtitle="Latest automatically captured Grade 12 outcomes for this county.">
          {!grade12Summaries.length ? (
            <Text style={styles.empty}>No Grade 12 result summaries have been created for this county yet.</Text>
          ) : (
            <View style={{ gap: 10 }}>
